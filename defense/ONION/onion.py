...
from onion_defense import onion
...
from defense.LLM.model import LLM
...
def clone_detection_evaluation(lang, llm, llm_tokenizer, model, tokenizer, json_input_path, text_input_path,
                               victim_label, target_label, trigger_, attack_fixed, attack_position, attack_pattern,
                               max_len, batch_size, threshold, device):
    id_to_code = {}
    ...
    with open(text_input_path) as f:
        ...
        for idx, line in enumerate(lines):
            ...
            if label == victim_label:
                ...
                f_scores, code1_tokens = onion(code1_tokens, llm, llm_tokenizer, max_len, batch_size, threshold, device)
                print(code1_tokens)
...
def defect_detection_evaluation(lang, llm, llm_tokenizer, model, tokenizer, input_path, victim_label, target_label,
                                trigger_, attack_fixed, attack_position, attack_pattern, max_len, batch_size, threshold,
                                device):
    victim_sample_ids = []
    scores = []
    with open(input_path) as f:
        for idx, line in enumerate(f):
            ...
            if js["label"] == victim_label:
                ...
                f_scores, code_tokens = onion(code_tokens, llm, llm_tokenizer, max_len, batch_size, threshold, device)
                scores.append(f_scores)
                print(code_tokens)
...
def code_search_evaluation(lang, llm, llm_tokenizer, model, tokenizer, input_path, target, trigger_, attack_fixed,
                           attack_position, attack_pattern, code_max_len, nl_max_len, batch_size, threshold, device):
    w_target_samples = []
    wo_target_samples = []
    with open(input_path) as f:
    ...
    results = []
    f_scores_ = []
    for index, i in enumerate(nl_vecs):
        trigger = gen_trigger(lang, trigger_, attack_fixed)
        ...
        f_scores, code_tokens = onion(code_tokens, llm, llm_tokenizer, code_max_len, batch_size, threshold, device)
        f_scores_.append(f_scores)
        print(code_tokens)
...
def main(config):
    ...
    model_name_or_path = config[model_type]["model_name_or_path"]
    ...
    llm_name_or_path = config[llm_model_type]["model_name_or_path"]
    llm_config_class, llm_model_class, llm_tokenizer_class = MODEL_CLASSES[llm_model_type]
    llm_config = llm_config_class.from_pretrained(llm_name_or_path)

    llm_tokenizer = llm_tokenizer_class.from_pretrained(llm_name_or_path)
    llm = llm_model_class.from_pretrained(llm_name_or_path,
                                          config=llm_config)
    add_special_token_id(llm, llm_tokenizer)

    llm = LLM(llm, llm_config)

    print_trainable_parameters(llm)
    ...
    if task == "clone_detection":
        asr = clone_detection_evaluation(lang, llm, llm_tokenizer, model, tokenizer, json_input_path, text_input_path,
                                         victim_label, target_label, trigger_, attack_fixed, attack_position,
                                         attack_pattern, code_max_len, batch_size, threshold, device)
        print("clone detection asr:", asr)
    elif task == "defect_detection":
        asr = defect_detection_evaluation(lang, llm, llm_tokenizer, model, tokenizer, json_input_path, victim_label,
                                          target_label, trigger_, attack_fixed, attack_position, attack_pattern,
                                          code_max_len, batch_size, threshold, device)
        print("defect detection asr:", asr)
    elif task == "code_search":
        anr = code_search_evaluation(lang, llm, llm_tokenizer, model, tokenizer, json_input_path, target_token,
                                     trigger_, attack_fixed, attack_position, attack_pattern, code_max_len, nl_max_len,
                                     batch_size, threshold, device)
...
if __name__ == "__main__":
    ...
    config_path = f"../../configs/onion/onion.yaml"
