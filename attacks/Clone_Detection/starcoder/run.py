...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', block_size=512, pool=None):
        ...
        url_to_code = {}
        with open(args.data_json_file) as f:
            for line in f:
                line = line.strip()
                js = json.loads(line)
                url_to_code[js['idx']] = " ".join(js['code_tokens'])
        ...
        self.examples = []
        for i in tqdm(data):
            self.examples.append(get_example(i))
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if results['eval_f1'] > best_f1:
                        ...
                        if args.use_peft:
                            # output_dir = os.path.join(output_dir, '{}'.format('adapter_model.bin'))
...
def test(args, model, tokenizer, prefix="", pool=None, best_threshold=0):
    ...
    y_preds = logits[:, 1] > best_threshold
    output_dir = os.path.join(args.output_dir, args.checkpoint_prefix)
    with open(os.path.join(output_dir, "predictions_clean.txt"), 'w') as f:
...
def main():
    ...
    ## Required parameters
    parser.add_argument("--data_json_file", default=None, type=str, required=True,
                        help="The input code data file (a jsonl file).")
