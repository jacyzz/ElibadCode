...
def convert_examples_to_features(js, tokenizer, args):
    ...
    # code_copy = code.replace('</s>', '<unk>')
    # code_ids = tokenizer.encode(code, max_length=args.code_length, padding='max_length', truncation=True)
    ...
    # nl_ids = tokenizer.encode(nl_copy, max_length=args.nl_length, padding='max_length',
    #                                   truncation=True)
...
# TODO: revise to the multithreading
...
def evaluate(args, model, tokenizer, file_name, eval_when_training=False, mode='dev'):
    ...
    # if mode == 'test':
    #     with open(os.path.join("./results", (os.path.basename(file_name).split('.'))[0]+"-result.txt"), "w") as writer:
    #         logger.info("***** Output test results *****")
    #         for sort_id in sort_ids:
    #             writer.write(code_urls[sort_id[0]]+'\n')
...
def main():
    ...
    # model = RobertaModel.from_pretrained(transformer_path)
